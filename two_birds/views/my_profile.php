	...
	?>
		<div class="container" id="origen" style="display:block; background-image: url(img/pbg.jpg); background-repeat: no-repeat; background-size: cover">
				...
				<div class="col-lg-4 border border-dark" style="margin-top:6%; background-image: url(img/profile_bg.jpg)">
					...
					<list>
						<ul>
							...
							<br>
							<li>Descripcion:<?php echo $_SESSION['usuario']['descripcion']; ?>
								<i class="bi bi-pencil-fill" onclick="descripcion()" style="cursor:pointer;"></i>
							</li>
						</ul>
					</list>
				</div>
			...
			<div class="row" style="display:none;" id="descripcion">
				...
				<form method="POST">
					<br>
					<textarea style="resize:none; margin-left:10%; width:80%; height:200px;" name="desc"><?php echo $_SESSION['usuario']['descripcion']; ?></textarea>
					<br>
					<br>
					<br>
					<p style="text-align:center;"><input type="button" value="Cancelar" name="eliminar" onclick="desc_c()" style="width:15%; background-color:red; color:white; ">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type="submit" value="Guardar" style="width:15%; background-color:green; color:white; "></p>
				</form>
			</div>
			...
			<div class="row" style="display:none;" id="buzon">
				<!-- Blog entries-->
				<h2>Buzon</h2>
				<h5>Mensajes: <?php echo count($moderacion) + count($comentarios); ?></h5>
				<br>
				<br>
				<br>
				<br>
					...
					foreach ($moderacion as $e) { ?>
						<div class="card">
							...
							<div class="card-body">
								...
								<p class="card-text"><?php if ($e['tipo'] == "informe") {
															echo "Informe";
														} else {
															echo "comentario";
														} ?> eliminado: <?php echo $e['titulo'] ?></p>
								<p class="card-text">Por: <?php echo $e['user_name'] ?></p>
							</div>
						</div>
						<br>
						<br>
				...
				if (count($comentarios) > 0) {
					foreach ($comentarios as $e) { ?>
						<div class="card">
							...
							<div class="card-body">
								<form method="POST">
									<input type="submit" name="msjc_d" value="x" class="bg-danger" style="float:right;">
									<input type="hidden" name="id" value="<?php echo $e[2]; ?>">
								</form>
								<h5 class="card-title">Comentario</h5>
								<p class="card-text"><?php echo $e[1] ?></p>
								<p class="card-text">Del informe: <a href="squeak.php?id=<?php echo $e[4]; ?>"><?php echo $e[0] ?></a></p>
								<p class="card-text">Por: <?php echo $e[3] ?></p>
							</div>
						</div>
						<br>
						<br>
				...
				if (count($comentarios) == 0 && count($moderacion) == 0) { ?>
...
<?php
	...
	<div class="container" id="origen" style="display:block; background-image: url(img/pbg.jpg); background-repeat: no-repeat; background-size: cover">
		<div class="row bg-light" style="font-size:20px; text-align:center;">
			<p style="margin-top:1%;"><a id="nav_principal" onclick="principal_aparecer()" style="cursor:pointer; text-decoration:underline;">Pagina principal</a>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<a id="nav_informes" onclick="informes_aparecer()" style="cursor:pointer; text-decoration:none;">Informes</a></p>
		</div>
		<div class="row" id="principal" style="display:flex;">
			<!-- Blog entries-->
			<div class="col-lg-8">
				<h2><?php echo $usuario['user_name']; ?></h2>
				<br>
				<img class="rounded-circle" src="img/<?php echo $usuario['foto']; ?>" id="foto" style="object-fit:cover; border:solid 5px #7EBC12; border-radius:20%;">
				<br>
				<br>
				<br>
			</div>
			<div class="col-lg-4 border border-dark" style="margin-top:6%; background-image: url(img/profile_bg.jpg)">
				...
				<list>
					<ul>
						<li>Informes subidos: <?php echo $cant_inf; ?></li>
						<br>
						<li>Fecha de creación de la cuenta: <?php echo $usuario["mes"] . " " . $usuario["dia"] . ", " . $usuario["Year"] ?></li>
						<br>
						<li>Descripcion: <?php echo $usuario["descripcion"]; ?></li>
					</ul>
				</list>
			</div>
		</div>
...
?>
<?php if (isset($_SESSION['usuario']) && !isset($_GET['id'])) { ?>
...
if (isset($_GET['id'])) { ?>
	<script src="js/perfil_v.js"></script>
<?php } ?>